        ...
        void RestoreCachedVertexAttributes(int index)
{
    ...
    if (index == -1 || index >= m_TextMeshPro.textInfo.characterCount) return;
    ...
    // Safely restore vertex attributes using helper method
    RestoreVertexAttributeData(materialIndex, vertexIndex);

    // Restore the last vertex attributes (last quad of vertices)
    int lastIndex = (m_cachedMeshInfoVertexData[materialIndex].vertices.Length / 4 - 1) * 4;
    RestoreVertexAttributeData(materialIndex, lastIndex);

    // Update all vertex data
    m_TextMeshPro.UpdateVertexData(TMP_VertexDataUpdateFlags.All);
}

void RestoreVertexAttributeData(int materialIndex, int vertexIndex)
{
    // Get cached data for vertices and colors
    Vector3[] src_vertices = m_cachedMeshInfoVertexData[materialIndex].vertices;
    Color32[] src_colors = m_cachedMeshInfoVertexData[materialIndex].colors32;

    // Get the destination data from the TextMeshPro text info
    ...
    // Ensure indices are within bounds before copying
    if (vertexIndex + 3 < dst_vertices.Length)
    {
        // Restore Vertices
